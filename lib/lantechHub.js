...
export async function reconstruct () {
  ...
  await reconstructUser()
  await reconstructProduct()
  await reconstructMib()
  await reconstructFirmware()
...
export async function reconstructUser () {
  ...
  process: { // eslint-disable-line
    ...
    // Special case - Jacky
    newUser['6BMKA3IUoKSC7rAUeJ8SRvwTY563'].custom = 'lantech'
    ...
    const newMibs = _.transform(currMibs, (result, mibInstance, versionKey) => {
      ...
      for (let {category, custom, l2, l3, model} of models) {
        for (let c of custom) {
          ...
          if (l3) result.download[`${normalize(c)}_${formattedVersion}_${model}_l3`] = currMibUrls[`${version.split('.').join('')}_${model}_l2`]
        }
      }
    }, {
      entry: {},
...
export async function reconstructFirmware () {
...
export async function clearPreviousHub () {
